...
def search_claim(param,claim):
    ...
    urls = []
    for result in search(claim, tld="com", num=param['top_search_results'], stop=param['top_search_results'], pause=randrange(5,10), lang='en', start=0,only_standard=False):   #   ==>>> using random number generator to generate the value of pause to avoid being blocked by google
        urls.append(result)
...
# justs put . in the end of article etc
...
def do_research(param):
    ...
    c=0
    for claim in claims:
        printLightPurple(" C L A I M     # "+str(c)+" ========================================================================")
        ...
        for a in relevant_articles:
            ...
            if a.text != "":
                ...
                # article.most_relevant_sent = [relevant sentence in order of similarity]
                # we only fill up rest of the article data structure for that article only if there are similar sentences in that article compared to the claim
        ...
        # claims = [(1,text,claimer, 23-23-3233, 2020),(2,text,claimer, 23-23-3233, 2020)]
        # claim.articles = [[article,article,article,article]]
        # filling up the Claim.articles[] for each claim to have all the articles that are analyzed
        # claims.articles = [relevantartcledatastructure,relevantartcledatastructure,relevantartcledatastructure] in the order of which article came on google search first
