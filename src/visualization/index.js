...
function changeColor(link) {
  ...
  // Perform the desired color change or other operations
}

fetch('/search', {
  method: 'POST',
  // Add form data if needed: body: new FormData(formData)
})
  .then(response => response.json())
  .then(json => {
    console.log(json);
    // Call your D3 code here and pass the JSON data
...
function processJSONData(json) {
  const edges = json.links.map(e => {
    const sourceNode = json.nodes.find(n => n.id === e.source);
    const targetNode = json.nodes.find(n => n.id === e.target);
